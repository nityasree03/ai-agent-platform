...
Runs the 30-scenario task dataset through the agent loop and scores:
  - Task Success Rate     — correct tool called + expected content found
  - Steps-to-Completion   — average planner/reflector steps per task
  - Tool-Call Accuracy    — % where the FIRST tool called matched expectation
  - Self-Correction Rate  — % of tasks that triggered at least one retry
  - Cost per Task         — average total tokens per task (proxy for $ cost)
...
RESULTS_HTML_PATH = Path(__file__).parent / "eval_report.html"
...
def run_eval() -> dict:
    ...
    for scenario in dataset:
        ...
        retried = state.get("self_correction_count", 0) > 0
...
def write_html_report(summary: dict):
    rows = "".join(
        f"<tr><td>{r['id']}</td><td>{r['query']}</td><td>{r['expected_tool']}</td>"
        f"<td>{r['actual_tool']}</td><td>{'OK' if r['success'] else 'FAIL'}</td>"
        f"<td>{r['steps']}</td><td>{r['self_correction_count']}</td></tr>"
        for r in summary["per_task"]
    )
    html = f"""<!DOCTYPE html>
<html><head><title>Agent Eval Report</title>
<style>
...
<body>
<h1>AI Agent Platform — Evaluation Report</h1>
<div>
  ...
  <div class="metric"><div class="value">{summary['avg_tokens_per_task']:.0f}</div><div class="label">Avg Tokens / Task</div></div>
</div>
<h2>Per-Scenario Results ({summary['n_scenarios']} scenarios)</h2>
<table>
<tr><th>#</th><th>Query</th><th>Expected Tool</th><th>Actual Tool</th><th>Success</th><th>Steps</th><th>Retries</th></tr>
{rows}
</table>
</body></html>"""
...
if __name__ == "__main__":
    ...
    print(f"Avg tokens/task:      {summary['avg_tokens_per_task']:.0f}")
    print(f"\nReports written to:")
